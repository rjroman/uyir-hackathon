...
# Route for live streaming video feed (optional)
@app.route('/video_feed')
def video_feed():
    return jsonify({"message": "Video feed available via RTSP or WebSocket."})

# Home route to verify the server is running
@app.route('/')
def home():
    return jsonify({"message": "Welcome to the Traffic Management System!"})

# Route for real-time webcam detection (using webcam)
@app.route('/webcam')
def webcam():
